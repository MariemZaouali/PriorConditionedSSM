...
CGNet (100 epochs) & 0.7367 & 0.8484 & 0.8626 & 0.8347 \\
\textbf{SPC-SSM (Ours)} & \textbf{0.7418} & \textbf{0.8606} & \textbf{0.8521} & \textbf{0.8693} \\ \hline
\textit{Improvement} & \textit{+0.51\%} & \textit{+1.22\%} & \textit{-1.05\%} & \textit{+3.46\%} \\ \hline
\end{tabular}
\end{table}

The results demonstrate that even with extended training and optimized hyperparameters, the CNN-based baseline cannot surpass our model. Specifically, our model maintains a lead in IoU and structural fidelity. 

Furthermore, we observe a critical difference in convergence stability. While the CGNet baseline eventually reached a high IoU, it exhibited extreme performance fluctuations towards the end of training—most notably a sharp dip of -4.6\% in IoU at epoch 99. In contrast, SPC-SSM maintained a much more stable and monotonic improvement curve throughout the 100 epochs. This suggests that the explicit semantic prior in SPC-SSM acts as a robust "stabilizing signal," effectively grounding the state-space transitions and preventing the erratic behavior observed in pure convolutional architectures.

\subsection{Qualitative Evaluation}
Visual analysis confirms that SPC-SSM effectively resolves the "internal holes" phenomenon in large building changes. By conditioning the state-space scan on a semantic prior, the model maintains a stable hidden state across homogeneous changed regions, while the selective scanning mechanism ensures sharp and accurate object boundaries. Unlike traditional concatenation-based fusion, which often results in fragmented change masks, our prior-guided selective scanning enforces spatial continuity, leading to significantly higher structural fidelity in complex urban environments.
