...
class PriorConditionedSelectiveStateSpace(nn.Module):
    def __init__(self, in_channels, hidden_dim=128, use_detach=True):
        ...
        # Projections sélectives (Mamba-style)
    ...
    def forward(self, x, prior):
        ...
        # Discrétisation
        A = -torch.exp(self.A).view(1, -1, 1, 1)
        A_bar = torch.exp(delta * A)
        B_bar = delta * B_k

        # 3. Parallel Prefix Scan — VECTORISÉ GPU
        def parallel_scan(feat, A_seq, B_seq, seq_dim):
            b = B_seq * feat
            P = torch.cumprod(A_seq, dim=seq_dim)
            P_safe = P.clamp(min=1e-8)
            h = P * torch.cumsum(b / P_safe, dim=seq_dim)
            return h

        # 4-Directional Cross-Scan (Bidirectionnel H et V)
        # Horizontaux (dim=3)
        h_fwd = parallel_scan(x_proj, A_bar, B_bar, seq_dim=3)
        h_bwd = parallel_scan(x_proj.flip(dims=[3]), A_bar.flip(dims=[3]), B_bar.flip(dims=[3]), seq_dim=3).flip(dims=[3])
