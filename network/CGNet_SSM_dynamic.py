...
# ============================================================
# 2. MODULE RPSS DYNAMIQUE (Avec Gating Apprenable)
# ============================================================
class RecursivePriorStateSpace(nn.Module):
    def __init__(self, in_channels, hidden_dim=128, use_detach=True):
        ...
        # Gating dynamique : décide de la confiance envers le Prior localement
        self.gate = nn.Sequential(
            nn.Conv2d(in_channels + 1, hidden_dim // 4, 3, padding=1),
    ...
    def forward(self, x, prior):
        ...
        # 1. Ajustement et Gating du Prior
        prior_up = F.interpolate(prior, size=(H, W), mode="bilinear", align_corners=True)
        prior_up = torch.clamp(prior_up, -1.0, 1.0)
        
        # Le masque de confiance appris
        mask = self.gate(torch.cat([x, prior_up], dim=1))
        F_mod = x + (mask * prior_up.expand(Bsz, C, H, W))
        ...
        def scan(feat, dim):
            ...
            for i in range(size):
                ...
                h = (B * x_i) if prev is None else (A * prev_val + B * x_i)
                steps.append(h)
                prev = h
...
class CGNet_SSM(nn.Module):
    def __init__(self):
        ...
        self.ssm3, self.ssm2, self.ssm1 = RecursivePriorStateSpace(512, 256), RecursivePriorStateSpace(256, 128), RecursivePriorStateSpace(128, 64)
...
if __name__ == "__main__":
    ...
    imgA, imgB = torch.randn(1, 3, 256, 256).cuda(), torch.randn(1, 3, 256, 256).cuda()
    coarse, fine = model(imgA, imgB)
    print(f"Modèle avec Gating Dynamique prêt. Output shapes: {coarse.shape}, {fine.shape}")
